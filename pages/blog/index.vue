  ...
  <main class="pt-14 max-w-[1200px] h-full">
      ...
      <div class="flex md:flex-col flex-wrap">
        <template v-for="doc in docs">
          <div class="hidden md:block dark:bg-gray-800 bg-gray-500 my-3 p-4 rounded-lg shadow-lg relative cursor-pointer" @click="navigateTo(doc.path)">
            ...
            <!-- <div class="inline-block ml-8 h-full align-top"> -->
              ...
              <p class="text-sm absolute bottom-0 right-0 p-6">{{ new Date(doc.date).toLocaleDateString() }}</p>
            <!-- </div> -->
          </div>
          ...
          <UCard class="mb-8 md:w-96 md:mx-2 h-max md:hidden" :ui="{ background: 'dark:bg-zinc-800 bg-zinc-200'}" >
            ...
            <template #footer>
              <div class="max-w-max ml-auto">
                ...
                <UButton :to="doc._path" icon="i-tabler-arrow-big-right-lines" trailing class="" label="Read Post"></UButton>
              </div>
            </template>
          </UCard>
        </template>
